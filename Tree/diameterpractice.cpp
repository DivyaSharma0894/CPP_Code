#include<iostream>
#include<queue>
#include<climits>
using namespace std;
class Node{
    public:int data;
    Node* left;
    Node* right;
    Node(int val){
        data=val;
        left=NULL;
        right=NULL;
    }
};
class tree{
    Node* root;
    public:
    tree(){
        root=NULL;
    }
   Node* getroot(){
    return root;
    }
    void insert(int val){
        Node* ans=new Node(val);
        Node* temp=root;
        if(root==NULL){
            root=ans;
            return;
        }
        queue<Node*>q;
        q.push(root);
        while(!q.empty()){
            temp=q.front();q.pop();
            if(!temp->left){temp->left=ans;return;}
            else{q.push(temp->left);}
             if(!temp->right){temp->right=ans;return;}
            else{q.push(temp->right);}
        }

    }
    int Findmax(Node* a){
        if(a==NULL){
            return INT_MIN;
        }
        int lef=Findmax(a->left);
        int rig=Findmax(a->right);
        return max(a->data,max(lef,rig));

    }
    int Findmin(Node* b){
        if(b==NULL){
            return INT_MAX;
        }
        int lef=Findmin(b->left);
        int rig=Findmin(b->right);
        return min(b->data,min(lef,rig));
    }
    void print(){
        Node* temp=root;
        if(root==NULL){
            cout<<"empty";
        }
        queue<Node*>q;
        q.push(root);
        while(!q.empty()){
            temp=q.front();q.pop();
            cout<<temp->data<<" ";
            if(temp->left)q.push(temp->left);
            if(temp->right)q.push(temp->right);
        }
        cout<<endl;
    }
};
int main(){
tree T;
 T.insert(10);  // root
    T.insert(20);  // left of 10
    T.insert(30);  // right of 10
    T.insert(40);  // left of 20
    T.insert(50);  // right of 20 (leaf)
    T.insert(60); 
    T.print();
    cout<<T.Findmax(T.getroot())<<endl;
    cout<<T.Findmin(T.getroot());
}